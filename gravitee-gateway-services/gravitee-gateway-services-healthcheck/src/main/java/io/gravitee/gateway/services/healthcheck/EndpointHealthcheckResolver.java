...
public class EndpointHealthcheckResolver {
    ...
    public List<EndpointRule> resolve(Api api) {
        ...
        if (rootHealthCheck != null && rootHealthCheck.isEnabled()) {
            ...
            return endpoints.map((Function<Endpoint, EndpointRule>) endpoint -> new DefaultEndpointRule(api.getId(), endpoint,
                    (endpoint.getHealthCheck() == null || endpoint.getHealthCheck().getTrigger() == null) ?
